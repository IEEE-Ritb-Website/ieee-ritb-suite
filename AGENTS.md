...
graph TB
    subgraph Packages["📦 Shared Packages & Clients"]
        ...
        CORE["📦 astranova-core<br/><i>Core Helpers & isProduction</i>"]
        ...
        SC["🔌 shared-clients<br/><i>Unified Client Singletons</i>"]
    end
...
The repository is organized cleanly into five core sections:
...
│   ├── 📦 astranova-core/           # Environment detection and root helpers
│   └── 📚 catalogues/               # Single-source-of-truth IEEE Chapter registry
├── 🔌 shared-clients/                # Centralized instantiated API client singletons (with client-config.json)
├── 🔧 services/
...
### 6. `📦 astranova-core`
*   **Location:** `file:///d:/ieee-ritb-suite/packages/astranova-core`
*   **Purpose:** Shared core environment and utilities. Exposes `isProduction()` for browser/server environments and `/node` entrypoint for Node-only helpers like `getMonorepoRoot()`.

### 7. `🔌 shared-clients`
*   **Location:** `file:///d:/ieee-ritb-suite/shared-clients`
*   **Purpose:** Centralized, pre-instantiated API client singletons (`AdminServiceClient`, `CommonAppServiceClient`, `RootServiceClient`) and local development connection checks. Auto-generated by running `pnpm run gen-client` in any backend service. Reads deployed URLs from `shared-clients/client-config.json`.
...
# To scaffold a new, complete Next.js + TypeScript + Tailwind v4 frontend application:
pnpm rs create-next <application-name>
```
