...
const getFormResponsesCollection = () =>
  getDb().collection<IFormResponse>("form-response");
...
export const submitFormResponse = async (req: Request, res: Response) => {
  ...
  const registrationType =
    ...
    (event.registrationConfig.type === "both"
      ? "individual"
      : event.registrationConfig.type);

  // extract team info if applicable
  let teamInfo = undefined;
  if (registrationType === "team") {
    const teamName =
      submissionData.teamName || submissionData.formData?.teamName;
    const teamSize =
      submissionData.teamSize || submissionData.formData?.teamSize;

    if (!teamName || !teamSize) {
      throw new ValidationError(
        "Team name and team size are required for team registration",
      );
    }
    ...
    if (teamConfig) {
      if (teamSize < teamConfig.minSize) {
        throw new ValidationError(
          `Team must have at least ${teamConfig.minSize} members`,
        );
      }
      if (teamSize > teamConfig.maxSize) {
        throw new ValidationError(
          `Team cannot exceed ${teamConfig.maxSize} members`,
        );
      }
    }
    ...
    teamInfo = {
      ...
      teamMembers:
        submissionData.teamMembers || submissionData.formData?.teamMembers,
    };
  }
  ...
  const result = await getFormResponsesCollection().insertOne(
    validatedResponse as any,
  );
