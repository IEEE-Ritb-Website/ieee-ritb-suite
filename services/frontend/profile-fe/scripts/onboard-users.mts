...
async function generateUniqueUsername(name: string, email: string, db: any): Promise<string> {
  const emailPrefix = email.split("@")[0].toLowerCase().replace(/[^a-z0-9_]/g, "");
  const nameParts = name.toLowerCase().split(/\s+/).map(p => p.replace(/[^a-z0-9_]/g, "")).filter(Boolean);
  ...
  if (nameParts.length > 0) {
    candidates.push((nameParts[0] + Math.floor(10 + Math.random() * 90)).substring(0, 20));
  }
  ...
  let attempt = 1;
  while (attempt <= 100) {
    const candidate = `${base}${Math.floor(100 + Math.random() * 900)}`.substring(0, 20);
    const exists = await db.collection("user").findOne({ username: candidate });
    if (!exists) {
      return candidate;
    }
  ...
  // 6. Fallback
  return (base + "_" + Math.random().toString(36).substring(2, 7)).substring(0, 20);
}
...
async function onboardUser(userData: {
  ...
  department: string;
  term: string;
}): Promise<{ success: true; email: string } | { success: false; email: string; error: string }> {
  ...
  if (!/^\d{9}$/.test(membershipId)) {
    console.error(`Validation Error (${userData.email}): Membership ID '${membershipId}' is invalid. It must be exactly 9 digits.`);
    return { success: false, email: userData.email, error: `Membership ID '${membershipId}' must be exactly 9 digits.` };
  }

  // Validate uniqueness of email
  const emailLower = userData.email.trim().toLowerCase();
  const existingEmail = await db.collection("user").findOne({ email: emailLower });
  if (existingEmail) {
    console.error(`Validation Error (${userData.email}): Email is already in use.`);
    return { success: false, email: userData.email, error: `Email '${userData.email}' is already in use.` };
  }

  // Validate uniqueness of membershipId
  const existingMId = await db.collection("user").findOne({ membershipId });
  if (existingMId) {
    console.error(`Validation Error (${userData.email}): Membership ID '${membershipId}' is already in use.`);
    return { success: false, email: userData.email, error: `Membership ID '${membershipId}' is already in use.` };
  }

  // Generate unique username dynamically (verifying uniqueness in database)
  const username = await generateUniqueUsername(userData.name, userData.email, db);

  // Map chapters
  const chapters: any[] = [];
  if (userData.chapter1) {
    const pos1 = userData.position && userData.position.trim() !== "" ? userData.position.trim() : "Execom";
    chapters.push(resolveChapter(userData.chapter1, pos1));
  }
  if (userData.chapter2) {
    chapters.push(resolveChapter(userData.chapter2, "Execom"));
  }
  ...
  if (!user) {
    console.error(`Error (${userData.email}): Failed to create user - no user object returned`);
    return { success: false, email: userData.email, error: "Failed to create user - no user object returned" };
  }
...
async function main() {
  ...
  if (!smtpOk) {
    console.error("Fatal Error: SMTP connection verification failed. Check your SMTP_HOST, SMTP_USER, and SMTP_PASS settings.");
    process.exit(1);
  }
  ...
  if (missingHeaders.length > 0) {
    console.error(`Fatal Error: File ${csvFile} is missing required headers: ${missingHeaders.join(", ")}`);
    process.exit(1);
  }
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (!/^\d{9}$/.test(mId)) {
      const rowIdentifier = user.email || user.name || "Unknown Row";
      console.error(`Row Validation Error (${rowIdentifier}): Membership ID must be exactly 9 digits.`);
      results.push({ success: false, email: user.email || "unknown", error: `Membership ID '${mId}' must be exactly 9 digits.` });
      continue;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (i < users.length - 1) {
          ...
          console.log(`Waiting ${(waitMs / 1000).toFixed(1)}s before next email...`);
          await delay(waitMs);
        }
      ...
      console.error("Exiting immediately to prevent silent failures. Re-run for remaining users.\n");
