...
export const SectionBlock = ({ title, children }: { title: string, children: React.ReactNode }) => (
  <section className="border border-[rgba(0,255,157,0.25)] rounded-[4px] p-3.5 relative bg-[rgba(0,255,157,0.015)]">
    <div className="text-xs tracking-[0.18em] text-[rgba(200,255,232,0.45)] uppercase mb-2.5 flex items-center gap-2">
...
export const SkillTag = ({ label, color }: { label: string, color: 'blue' | 'green' | 'amber' | 'pink' }) => {
  ...
  return (
    <span className={`text-sm px-2 py-0.5 rounded-[2px] tracking-[0.06em] border ${styles[color]}`}>
      {label}
    </span>
  );
};
...
export const TimelineItem = ({ year, title, subtitle, badge, badgeColor }: { year: string, title: string, subtitle: string, badge?: string, badgeColor?: 'amber' | 'blue' }) => {
  ...
  return (
    <div className="grid grid-cols-[70px_1fr] gap-3 mb-3 text-xs">
      <div className="text-[#00ff9d] pt-0.5 tracking-[0.05em] text-xs">{year}</div>
      <div className="border-l border-[rgba(0,255,157,0.25)] pl-3 pb-2">
        ...
        <div className="text-[rgba(200,255,232,0.45)] text-xs leading-relaxed">{subtitle}</div>
      </div>
    </div>
  );
};
...
export const Badge = ({ label, color }: { label: string, color: 'amber' | 'green' | 'blue' | 'pink' }) => {
  ...
  return (
    <span className={`flex items-center gap-1.5 p-1 px-2.5 rounded-[2px] text-xs tracking-[0.05em] border ${styles[color]}`}>
      {label}
    </span>
  );
};
...
export const OpenTag = ({ label }: { label: string }) => (
  <div className="flex items-center gap-1.5 p-1 px-3 border border-dashed border-[rgba(0,255,157,0.35)] rounded-[2px] text-xs text-[#00ff9d] tracking-[0.05em]">
    <div className="w-1.5 h-1.5 rounded-full bg-[#00ff9d] animate-pulse-dot" />
    {label}
  </div>
);

export const ChapterChip = ({
  color,
  title,
  subtitle,
  onClick
}: {
  color: 'amber' | 'blue' | 'pink' | 'red',
  title: string,
  subtitle: string,
  onClick?: () => void
}) => {
  const styles = {
    ...
    red: "border-[rgba(239,68,68,0.35)] bg-[rgba(239,68,68,0.06)] text-[#ef4444]"
  };
  const dotColors = {
    amber: "bg-[#ffb700]",
    blue: "bg-[#00cfff]",
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-start gap-2 p-1.5 px-2.5 rounded-[3px] border text-xs tracking-[0.05em] transition-colors hover:bg-opacity-10 ${onClick ? 'cursor-pointer' : 'cursor-default'
        } ${styles[color]}`}
    >
      <div className={`w-1.5 h-1.5 mt-2 rounded-full flex-shrink-0 ${dotColors[color]}`} />
...
export const LinkItem = ({ label, icon, href = "#" }: { label: string, icon: React.ReactNode, href?: string }) => (
  <a href={href} target='_blank' className="flex items-center gap-2 text-sm text-[rgba(200,255,232,0.45)] no-underline py-1 border-b border-[rgba(0,255,157,0.07)] transition-colors hover:text-[#00ff9d] w-full">
