...
const PROJECT_TYPE_META: Record<string, ProjectTypeMeta> = {
  website: { Icon: Globe, label: "Website / Web App", bgColor: "rgba(0,207,255,0.06)", iconColor: "#00cfff", borderColor: "rgba(0,207,255,0.25)" },
  aiml: { Icon: Brain, label: "AI / ML Project", bgColor: "rgba(255,79,216,0.06)", iconColor: "#ff4fd8", borderColor: "rgba(255,79,216,0.25)" },
  cli: { Icon: Terminal, label: "CLI / Tooling", bgColor: "rgba(0,255,157,0.05)", iconColor: "#00ff9d", borderColor: "rgba(0,255,157,0.2)" },
  cybersecurity: { Icon: ShieldAlert, label: "Cybersecurity", bgColor: "rgba(255,183,0,0.06)", iconColor: "#ffb700", borderColor: "rgba(255,183,0,0.25)" },
  research: { Icon: FileText, label: "Research Paper", bgColor: "rgba(200,255,232,0.04)", iconColor: "rgba(200,255,232,0.6)", borderColor: "rgba(200,255,232,0.15)" },
  embedded: { Icon: Cpu, label: "Embedded / Hardware", bgColor: "rgba(255,79,216,0.05)", iconColor: "#ff4fd8", borderColor: "rgba(255,79,216,0.2)" },
  mobile: { Icon: Smartphone, label: "Mobile App", bgColor: "rgba(0,207,255,0.05)", iconColor: "#00cfff", borderColor: "rgba(0,207,255,0.2)" },
  game: { Icon: Gamepad2, label: "Game", bgColor: "rgba(255,183,0,0.06)", iconColor: "#ffb700", borderColor: "rgba(255,183,0,0.22)" },
  devtool: { Icon: Wrench, label: "Dev Tool / Library", bgColor: "rgba(0,255,157,0.06)", iconColor: "#00ff9d", borderColor: "rgba(0,255,157,0.22)" },
  other: { Icon: Lightbulb, label: "Other", bgColor: "rgba(200,255,232,0.03)", iconColor: "rgba(200,255,232,0.45)", borderColor: "rgba(200,255,232,0.12)" },
};

const BADGE_COLORS: Record<string, "amber" | "green" | "blue" | "pink"> = {
  hackathon: "amber",
  gsoc: "green",
  open_source: "pink",
  certification: "blue",
...
function ProjectDetailModal({ project, onClose }: { project: NonNullable<ProfileViewProps["data"]["projects"]>[0]; onClose: () => void }) {
  const meta = PROJECT_TYPE_META[project.type || "other"] || PROJECT_TYPE_META["other"];
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      ...
      >
        ...
        <div className="flex justify-between items-start px-5 py-4 border-b border-[rgba(0,255,157,0.2)] relative z-10"
        ...
        <div className="p-6 relative z-10 space-y-5">
          {longDesc && (
            <div>
              <div className="text-xs uppercase tracking-widest text-[rgba(200,255,232,0.35)] mb-2">About</div>
              <p className="text-sm text-[rgba(200,255,232,0.7)] leading-[1.8] whitespace-pre-wrap">{longDesc}</p>
            </div>
          )}
          ...
          {(primaryLink || extraLink) && (
            <div className="space-y-2">
              <div className="text-xs uppercase tracking-widest text-[rgba(200,255,232,0.35)] mb-2">Links</div>
              {primaryLink && (
                <a
                  href={primaryLink}
                  target="_blank"
                ...
                >
                  {isGithub(primaryLink) ? <Github size={13} /> : <ExternalLink size={13} />}
                  <span className="tracking-wider">{isGithub(primaryLink) ? "View on GitHub" : "View Resource"}</span>
                </a>
              )}
              {extraLink && (
                <a
                ...
                >
                  {isGithub(extraLink) ? <Github size={13} /> : <Link2 size={13} />}
                  <span className="tracking-wider">{isGithub(extraLink) ? "View on GitHub" : "Extra Link"}</span>
                </a>
              )}
            </div>
          )}
          ...
          {project.tags && project.tags.length > 0 && (
            <div>
              <div className="text-xs uppercase tracking-widest text-[rgba(200,255,232,0.35)] mb-2">Tags</div>
              <div className="flex flex-wrap gap-1.5">
                {project.tags.map(tag => (
                  <span key={tag} className="text-xs px-2 py-0.5 rounded-[2px] border border-[rgba(0,255,157,0.2)] text-[#00ff9d] opacity-60">
                    #{tag}
                  </span>
                ))}
              </div>
    ...
    <div className="flex flex-col divide-y divide-[rgba(0,255,157,0.08)]">
      {achievements.map((ach, idx) => {
        ...
        return (
          <div key={idx}>
            <button
              onClick={() => hasExtra ? setOpenIdx(isOpen ? null : idx) : undefined}
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                {ach.date && (
                  <span className="text-xs text-[rgba(200,255,232,0.35)] flex-shrink-0">· {ach.date}</span>
                )}
              </div>
              {hasExtra && (
                <ChevronDown
...
export const ProfileView = ({ data }: ProfileViewProps) => {
  ...
  const [selectedProject, setSelectedProject] = useState<NonNullable<ProfileViewProps["data"]["projects"]>[0] | null>(null);
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <SectionBlock title="SKILL STACK">
        {skills.length === 0 ? (
          <div className="text-xs opacity-40 italic uppercase tracking-wider">// no skill telemetry logged</div>
        ) : (
            ...
            {fallbackSkills.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-1">
                {fallbackSkills.map((skill, idx) => {
                  const colors: ('blue' | 'green' | 'amber' | 'pink')[] = ['blue', 'green', 'amber', 'pink'];
                  return <SkillTag key={skill} label={skill} color={colors[idx % colors.length]} />;
                })}
              </div>
            )}
          </div>
      ...
      <SectionBlock title="HONORS & ACHIEVEMENTS">
        {validAchievements.length > 0 ? (
          ...
          <div className="text-sm opacity-40 italic uppercase tracking-wider">// no validated honors recorded</div>
        )}
      </SectionBlock>
        ...
        {validProjects.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {validProjects.map((project, idx) => {
              ...
              const { Icon, bgColor, iconColor, borderColor, label } = meta;
              const primaryLink = project.primary_link || project.link;
              const isGithub = primaryLink && primaryLink.includes("github.com");
                  ...
                  <div className="flex items-center justify-between mt-2">
                    {project.tags && project.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        ...
                        {project.tags.length > 3 && <span className="text-xs opacity-25 text-[rgba(200,255,232,0.4)]">+{project.tags.length - 3}</span>}
                      </div>
                    )}
